Namespace IS314_Proj1_Kernja
    Public Class clsCategoryFactory
        ...
        End Sub
        Public Sub addCategory(ByVal passedName As String, ByVal passedMin As Integer, ByVal passedMax As Integer, ByVal passedProbation As Integer, ByVal passedValue As Integer)
            'add a new Category with a passed name with specifically set values
            _hash.Add(passedName, New clsCategory(passedName, passedMin, passedMax, passedProbation, passedValue))
        End Sub
        ...
        Private Sub doSpecialFeatures(ByRef passedCategory As clsCategory)
            ...
            'IGNORE THIS WASPNEST OF BAD CODE!!!!!
            'It will change the overall code design to the lower of logic and datatypes
            'It will be replaced since this is BAD CODING!!!!
            Dim tempIndex As Integer = 0
            Dim tempCategory(0 To 2) As clsCategory
            tempCategory(2) = passedCategory
            If tempCategory(2).Name = "Overall Code Design" Or tempCategory(2).Name = "Code Logic" Or tempCategory(2).Name = "Code Data Types" Then
                tempCategory(2) = Me.getCategory("Overall Code Design")

                tempCategory(0) = Me.getCategory("Code Logic")
                tempCategory(1) = Me.getCategory("Code Data Types")
                Dim tempValue As Integer = tempCategory(0).Value
                If tempCategory(1).Value < tempValue Then
                    tempValue = tempCategory(1).Value
                    tempIndex = 1
                End If

                If tempCategory(2).Value < tempValue Then
                    Dim myResponse As MsgBoxResult = MsgBox("Would you care to raise the value of '" & tempCategory(2).Name & "' to the value of '" & tempCategory(tempIndex).Name & "'?", MsgBoxStyle.YesNo, "Question")
                    If myResponse = MsgBoxResult.Yes Then
                        tempCategory(2).Value = tempValue
                    End If
                End If

                If tempCategory(2).Name.Equals(tempCategory(2).Name) Then
                    tempCategory(2).Value = tempCategory(2).Value
                End If
            End If
